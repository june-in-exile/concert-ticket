...
export const TicketNFT = ({ children }) => {
    const buyOnChain = async () => {
        ...
        const contract = new ethers.Contract(process.env.CONTRACT_ADDRESS, ticketNFT.abi, signer)
        try {
    ...
    const validateOnChain = async (ticketId: number) => {
        const provider = new ethers.JsonRpcProvider();
        const contract = new ethers.Contract(process.env.CONTRACT_ADDRESS, ticketNFT.abi, provider)
    ...
    return (
        <TicketNFTContext.Provider value={{ buyOnChain, validateOnChain, cancelOnChain }}>
            {children}
